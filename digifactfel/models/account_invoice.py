        ...
        try:
            for factura in self:             
                if (
                    ...
                    nombre_emisor = factura.company_id.company_registry
                    ...
                    items = []
                    taxes_totales = []
                    # search if line name  = % y el precio es negativo
                    linea_negativa = list(filter(lambda x: x.price_unit < 0, factura.invoice_line_ids))
                    if len(linea_negativa) > 0:
                        linea_negativa = linea_negativa[0]

                    for line in factura.invoice_line_ids:
                        if linea_negativa == line:
                            logging.info("linea negativa encontrada!")
                            continue
                        numero_linea = 1
                        ...
                        precio = precio_unitario * line.quantity
                        ...
                        if linea_negativa:                             
                            if "%" in linea_negativa.product_id.name:                                 
                                porcentaje_descuento = linea_negativa.product_id.name.split("%")[0]
                        ...
                        if porcentaje_descuento > 0:                             
                            descuento += float("{:.3f}".format(precio_unitario * (porcentaje_descuento / 100)))                             
                            precio_subtotal = "{:.3f}".format(precio_unitario * (porcentaje_descuento / 100))
                            total = float("{:.5f}".format(precio - descuento))
                        ...
                        if len(line.tax_ids) > 0 and tipo != "FPEQ":
                            ...
                            for tax in taxes["taxes"]:
                                ...
                                if porcentaje_descuento > 0:
                                    monto_gravable = round(float(total / 1.12), 2)
                                    monto_impuesto = round(total - monto_gravable, 2)
